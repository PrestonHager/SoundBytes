  ...
  if (typeof src === 'object' && src.hook && src.dist) {
    ...
    return new Promise((resolve, reject) => {
      exec(src.hook, options, (err) => {
        if (err) return reject(new Error(`Failed running "src.hook": "${src.hook}" due to the following error: "${err.stderr}"
        ${err.stdout}`))
...
const preRunSrcUpload = async (sdk, orgName, src) => {
  ...
  // Zip source
  await utils.zip(src, packagePath)
  ...
  let resultUpload // eslint-disable-line
  try {
    resultUpload = await request.put(packageUrls.upload, body, config)
  } catch (e) {
    throw e
  }
...
const run = async (sdk, method = {}, instanceData = {}, credentials = {}, options = {}) => {
  ...
  if (instance.inputs.src) {
    ...
    instance.inputs.src = await preRunSrcUpload(sdk, instance.orgName, instance.inputs.src)
  }
